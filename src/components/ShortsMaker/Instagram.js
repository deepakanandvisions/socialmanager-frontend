...
const Instagram = ({ ShortsMakerField }) => {
    ...
    const checkLoginStatus = () => {
        const token = localStorage.getItem('jwtInstaToken');
        if (!token) {
        ...
        axios.get(apiUrl + '/auth/login/insta_status', {
            headers: {
            ...
            .catch(error => {
                ...
                localStorage.removeItem('jwtInstaToken');
                setIsLoggedIn(false);
                setIsLoading(false); // Update loading status
            });
    };
    ...
    const fetchUserInfo = () => {
        const token = localStorage.getItem('jwtInstaToken');
        axios.get(apiUrl + '/auth/insta_userinfo', {
            headers: {
                Authorization: `Bearer ${token}`
            }
        })
    ...
    useEffect(() => {
        ...
        if (token) {
            localStorage.setItem('jwtInstaToken', token);
            setIsLoggedIn(true);
        }
        checkLoginStatus();
    }, []);
